...
public class MyController {
	...
	public void join(HttpServletRequest request, @RequestParam("email") String email,
			@RequestParam("password") String pw, @RequestParam("nickname") String nickname,
			@RequestParam("phonenumber") String phoneNumber, @RequestParam("MEMBER_IMAGE") String image) {
	...
	// 안드로이드 로그인이메일에 따른 이름값 출력
	@RequestMapping("/Login_name")
	public String Login_name(Model model,@RequestParam("Email") String email) {
		
		System.out.println("Login name test");
		
		String loginName = dao.loginName(email);
